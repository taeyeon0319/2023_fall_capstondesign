...
const HelperDetail = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_SERVER_URL}/helper/users/${userInfo.id}`)
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_SERVER_URL}/review/helper-review/${helper_id}`
      )
      .then((res) => {
      ...
      return (
        <div className="review-item">
          <div className="left">
            ...
            <span style={{ textAlign: "center", width: "55px" }}>
              ★ {review.rating}
            </span>
          </div>
          <div className="right">
  ...
  const requestHelper = () => {
    console.log("reqest!");
    axios
      .post(`${process.env.REACT_APP_SERVER_URL}/helper/requests/add`, {
        user_id: userInfo.id,
      ...
      <div className="helper-list-container">
        <div className="helper-search-form">
          <div className="helper-list-searched-item">
            ...
            <div className="description">
              <h4 className="name">
                {helperInfo.name}{" "}
                <span style={{ fontSize: "13px" }}>도우미</span>
              </h4>
              <p className="region">
        ...
        {!secondStep && (
          <div className="helper-list-searched-container">
            <div className="title">
              <span className="fl">후기</span>{" "}
              <span className="count ft-size15 fr">
                평균{" "}
                {reviewList.reduce((sum, curValue) => {
                  return (
                    Math.round(
                      (sum + curValue.rating / reviewList.length) * 100
                    ) / 100
                  );
                }, 0)}
                점 | 총 {reviewList.length}건의 후기
              </span>
            </div>

            <div className="review-list">{getReviewList()}</div>
          </div>
        )}
        {secondStep && (
          <div className="helper-list-searched-container">
              ...
              <ul className="filter-list">
                ...
                <li className="filter-list-item">
                  ...
                  <div className="select-container-4">
                    <TimePicker
                      placeholder="00:00"
                      defaultValue={dayjs(`${date} ${serviceStartTime}:00`)}
