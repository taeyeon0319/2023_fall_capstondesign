    ...
    <Root>
      ...
      <HelperMyProfileBox>
        ...
        >
          ...
          null ? (
            <HelperMyText3>지역을 등록해주세요</HelperMyText3>
          ) : (
            <HelperMyText3>
              {JSON.parse(localStorage.getItem("userInfo")).region_state +
                " " +
            ...
            {accepted.map((item, index) => (
              <UserHelperList style={{ justifyContent: "space-between" }}>
                <div style={{ display: "flex", width: "80%" }}>
                  ...
                  >
                    <UserPText5>
                      {item.name} {item.date.substr(0, 10)}
                    </UserPText5>
                    <UserPText6 style={{ display: "flex" }}>
                      도움 시간{" "}
                      <p style={{ marginLeft: "0.8vh", fontWeight: "700" }}>
